...
<script>
    ...
    function bbsFormCheck(){
        ...
        if(document.getElementById("fie").name=="mf"&&document.getElementById("fie").value=="") {
        	alert("파일을 넣어주세요");
        	return false;
        }
    ...
    function newFileSelect() {
    	...
    	document.getElementById("fie").name="mf";
    }
    ...
    <h1>자료 올리기 폼</h1>
     ...
     <form method="post" action="<%= request.getContextPath()%>/data/dataEditOk" 
        ...
        <div>제목</div>
        <div><input type="text" name="subject" id="subject" value="${vo.subject }"></div>
        <div>글내용</div>
        <div><textarea name="content" id="content">${vo.content }</textarea></div>
        <div>첨부파일</div>
        <div id="fileView">${vo.filename } <b onclick="newFileSelect()">X</b></div>
        <div id="fileSelect" style="display:none;"><input type="file" name="" id="fie"></div>
        <div class="btn">
